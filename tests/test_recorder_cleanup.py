...
def _camera(**record_overrides) -> CameraConfig:
    ...
    return CameraConfig(id="front_door", name="Front Door", type="rtsp",
                        rtsp_url="rtsp://x", record=record)
...
def test_files_kept_when_disk_has_space(tmp_path, disk_usage):
    ...
    rec = CameraRecorder(_camera(delete_only_if_disk_full=True, min_disk_free_gb=10.0),
...
def test_strict_mode_deletes_by_age(tmp_path, disk_usage):
    ...
    rec = CameraRecorder(_camera(delete_only_if_disk_full=False, retain_days=30),
                         recordings_dir=tmp_path)
...
def test_manager_skips_disabled_cameras(tmp_path):
    ...
    cams = [_camera(), CameraConfig(id="mac", name="Mac", type="macbook",
